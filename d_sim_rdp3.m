...
sand_d = sand_d * 0.000000001; %[kg/mm^3]
...
omega = deg2rad(10) ;%wheel angular speed [deg/s]
duration = deg2rad(180) / (omega) ; %test time
dt = duration / n ;
q = 1;
k =1;
theta = (linspace(pi/4,0,250)).';%unit rad
for i=1:n
    if i < 499
        xS(i) = w_x(i,1);
        yS(i) = y;
    ...
    if i > 499 && i < 749
        xS(i) = xS(i-1)  +sqrt(200) /250 + w_x(i,1) ;
        yS(i) = yS(i-1) +sqrt(200) /250;
    end
    if i > 749 
        xS(i) =rS*cos(theta(q)) + w_x(i,1);
        yS(i) =rS*sin(theta(q));
        q     = q +1;
    end
...
%             ForceZ(i,q) = sigma_z(i,q) * TreadWidth * seg_length *sin(B(i,q)) * 0.01  ;
            Dragforce(i,q) = 0.5 * (d_vx(i,q))^2 * sand_d * seg_length *sin(B(i,q))* TreadWidth * Cd  ;
         else
             ForceX(i,q) = 0 ;
             ForceZ(i,q) = 0 ;
             Dragforce(i,q) =0;
 ...
 if ani == 1
    ...
    open(writeObj);
     for tdx= 1:n
          ...
          plot(xP(1:tdx,end),yP(1:tdx,end),'-o','Color','b','Markersize',5,'MarkerFaceColor','b');
          daspect([1 1 1]);
