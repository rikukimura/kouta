...
# y軸方向の描画幅を指定
ax.set_ylim((-1.1, 1.1))
li, = plt.plot(times, Fzs,label = 'Fx')
li2,= plt.plot(times, Fys,label = 'Fy')
#######################################################↑ Force sensor
sock.sendto(start_command,atiAddress)
...
while j <= 20:
    ...
    Fz = Fz[0]/float(1000000)
    Fy = (1.6 * Fy[0])/float(1000000)
    ...
    offsets_y.pop(0)
    j = j + 1
off_z = sum(offsets_z)/20
off_y = sum(offsets_y)/20
print('offset value Holizon: = %f' %off_y)
print('offset value Vertical: = %f' %off_z)
...
with open(path+basename+'.csv','w') as logFile:
    ...
    while 1:
        ...
        # convert binary data to float
            ...
            Fz = Fz[0]/float(1000000)  - off_z
            Fz = - Fz
            Fy = (1.6 * Fy[0])/float(1000000)  - off_y

            logFile.write('%f,%f,%f,%f'%(t,Fx[0]/float(1000000),Fy,Fz))
            logFile.write('\n')
          ...
          # print(Fzs)
            ...
            #plt.plot(times, Fzs, label="test")
            li2.set_xdata(times)
            li2.set_ydata(Fzs)

            #plt.xlim(min(times),max(times))
            plt.xlim(0,18)
            plt.ylim((min(Fys)-0.5),(max(Fys)+0.5))
            plt.xlabel("time[s]")
            plt.ylabel("Force[N]")
